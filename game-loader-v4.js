(() => {
  ...
  const SANS_FIDELITY_V12_URL = `sans-fidelity-v12.js?v=${VERSION}`;
  ...
  function executeGame(source) {
    return new Promise((resolve, reject) => {
      const blob = new Blob([`${source}\n//# sourceURL=game-sans-fidelity-v12.js`], { type: 'text/javascript' });
...
`);
  ...
  async function start() {
    try {
      ...
      await loadExternalScript(SANS_FIDELITY_V12_URL);
      ...
      const required = [
        ...
        ['applySansFidelityV12', 'Sans fidelity v12']
      ];
      ...
      source = window.applySansFidelityV12(source);
      window.__UNDERTALE_BUILD__ = VERSION;
      console.info('UNDERTALE build ready:', VERSION);
